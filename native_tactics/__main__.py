...
def bootstrap_audio_runtime() -> None:
    ...
    if all(path in current_paths for path in required_paths) and os.environ.get("SDL_AUDIODRIVER") in {"pulseaudio", "pulse"}:
        return
    ...
    os.execvpe(sys.executable, [sys.executable, "-m", "native_tactics", *sys.argv[1:]], env)
